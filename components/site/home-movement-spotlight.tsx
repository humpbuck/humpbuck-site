...
export function HomeMovementSpotlight({
  ...
  return (
    <div className="mx-auto flex w-fit max-w-full flex-col items-center gap-5 md:w-full md:max-w-none md:flex-row md:justify-center md:gap-14 lg:gap-20">
      <div className="flex w-full max-w-[240px] shrink-0 flex-col items-center md:mx-0 md:max-w-[280px] lg:max-w-[320px]">
        <Link
          href={productHref}
          className="group relative block aspect-square w-full overflow-hidden rounded-xl border border-white/15 bg-white/5 transition duration-300 hover:border-white/25 hover:bg-white/10"
        >
          <StorefrontImage
            src={activeImage}
            alt={imageAlt}
            ...
            className={
              productCutout
                ? "object-contain object-center p-3 transition duration-700 group-hover:scale-[1.03] md:p-4"
      ...
      <div className="flex w-fit max-w-full flex-col items-center rounded-2xl border border-white/15 bg-white/5 px-5 py-4 text-center sm:px-6 sm:py-5 md:min-w-0 md:max-w-xs md:flex-none md:items-start md:text-left">
        <p className="text-[9px] font-semibold uppercase tracking-[0.14em] text-white/88 md:text-[10px] md:tracking-[0.2em]">
          {kicker}
        </p>
        ...
        <Link
          href={productHref}
          className="mt-3 inline-flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-white/88 underline-offset-8 transition hover:text-white hover:underline md:mt-4 md:gap-2 md:text-[11px] md:tracking-[0.14em]"
        >
          {cta}
