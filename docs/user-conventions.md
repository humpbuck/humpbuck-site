...
| 2026-07-25 | **Products 页滑动提示**：分类胶囊与产品卡片变体改为横向滑动，溢出时下方显示细滑杆（`SubtleHorizontalScroll`）。 |
...
| 2026-09-02 | **PDP 打开加速**：路由 `loading` 骨架；页面壳子不再阻塞目录查询；去掉 PDP 内 `getSiteHomeContent`/`connection()`；店面目录 60s 缓存 + 后台商品/库存 `revalidateTag`；后台图齐时不再因空视频扫整桶 R2。 |
...
| 2026-09-03 | **首页 Recommended → 四分类轮播**：ANA-DIGI / DIGITAL / ANALOG / AUTOMATIC（空分类不显示）；**每类最多 12 款**；商家后台 Home product arrange 对应四个分类（仍保留 Featured）；Shop all 链到各分类落地页。 |
