...
import { CheckoutSuccessClient } from "@/app/(site)/checkout/success/CheckoutSuccessClient";
...
export default async function CheckoutSuccessPage({
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
      {purchaseAllowed ? <CheckoutSuccessClient orderId={order.id} totalUsd={order.totalCents / 100} provider={order.provider} /> : null}
      <div className="rounded-3xl border border-emerald-200 bg-emerald-50 p-6 sm:p-8">
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0">
            <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-emerald-700">Payment successful</p>
            <h1 className="mt-2 font-serif text-3xl tracking-tight text-ink">Your order is confirmed</h1>
            <p className="mt-2 text-sm leading-relaxed text-emerald-900/85">
              Thanks for your purchase via {providerLabel}. We’ve saved your order and you can review the details below.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 rounded-2xl border border-line bg-white/70 p-5">
        <div className="flex flex-wrap items-start justify-between gap-4 border-b border-line pb-4">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-muted">Order</p>
            <p className="mt-1 text-lg font-semibold text-ink">#{order.id.slice(-8)}</p>
            <p className="mt-3 text-xs text-muted">Placed {placedAt}</p>
          </div>
          <div className="text-right">
            <p className="text-2xl font-semibold tabular-nums text-ink">{formatPrice(order.totalCents / 100)}</p>
            <p className="mt-1 text-xs uppercase tracking-[0.14em] text-muted">{order.status}</p>
          </div>
        </div>

        <div className="mt-5">
          <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-muted">Items</p>
