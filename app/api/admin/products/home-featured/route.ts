...
} from "@/lib/catalog-home-featured";
...
export async function PATCH(req: Request) {
  ...
  if (productIds.length > MAX_HOME_FEATURED) {
    return NextResponse.json(
      { error: `Select at most ${MAX_HOME_FEATURED} products.` },
      { status: 400 },
    );
  }

  try {
    await setHomeFeaturedProducts(productIds);
    return NextResponse.json({ ok: true, productIds });
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    return NextResponse.json(
      { error: `Failed to update homepage featured: ${msg}` },
      { status: 500 },
    );
  }
}
